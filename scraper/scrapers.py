...
def _record_snippet(retailer_name: str, html: str):
    ...
    # don't center on false positives like jQuery's "$" or a CSS icon-font
    # glyph definition such as --fa:"\$" — those contain a dollar sign but
    # no digits, so they were slipping past a naive html.find("$").
    ...
    for attempt in range(1, max_retries + 1):
        try:
            with sync_playwright() as p:
                ...
                # Poll for up to ~10s for a "checking your browser"-style
                # interstitial to clear and redirect to the real page,
                # rather than a fixed short wait. Basic JS challenges
                # (AWS WAF, Cloudflare's non-interactive mode) often resolve
                # on their own within this window; interactive challenges
                # (Cloudflare Turnstile) generally will not, no matter how
                # long we wait — that's expected and handled below.
                for _ in range(5):
